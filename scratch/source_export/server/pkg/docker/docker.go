...
func SpawnZoneContainers(ctx context.Context, zoneID int) (string, error) {
	...
	gatewayHost := os.Getenv("GATEWAY_PUBLIC_HOST")
	if gatewayHost == "" {
		gpa := os.Getenv("GATEWAY_PUBLIC_ADDR")
		if gpa != "" {
			if idx := strings.Index(gpa, ":"); idx != -1 {
				gatewayHost = gpa[:idx]
			} else {
				gatewayHost = gpa
			}
	...
	return fmt.Sprintf("%s:%d", gatewayHost, gatewayPort), nil
}
