...
func (h *WorldHandler) SetFormation(ctx context.Context, req *pb.SetFormationRequest) (*pb.SetFormationResponse, error) {
	h.log.Info("SetFormation request")
	userID, ok := h.getUserID(ctx)
	if !ok {
		return &pb.SetFormationResponse{Base: &pb.BaseResponse{Code: 401, Message: "msg_unauthorized"}}, nil
	}

	slotsMap := make(map[int32]int64)
	for _, slot := range req.Slots {
		slotsMap[slot.Position] = slot.PlayerHeroId
	}

	code, msg := h.svc.SetFormation(ctx, userID, slotsMap)
	if code != 0 {
		return &pb.SetFormationResponse{Base: &pb.BaseResponse{Code: code, Message: msg}}, nil
	}

	return &pb.SetFormationResponse{
		Base: &pb.BaseResponse{Code: 0, Message: "msg_success"},
	}, nil
}
