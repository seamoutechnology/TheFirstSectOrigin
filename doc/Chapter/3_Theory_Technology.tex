...
Go là ngôn ngữ biên dịch trực tiếp ra mã máy (compiled language), giúp đạt tốc độ thực thi tiệm cận C/C++ mà không gặp độ trễ lớn như các ngôn ngữ thông dịch hoặc chạy trên máy ảo (như Java, C\#). Đặc biệt, Go cung cấp cơ chế xử lý đồng thời nhẹ dựa trên \textit{Goroutines} và kết nối giữa các luồng thông qua \textit{Channels}:
\begin{itemize}
    ...
    \item \textbf{Scheduler (M:N)}: Go Runtime phân phối hệ thống $M$ Goroutines lên $N$ luồng hệ điều hành một cách tự động và tối ưu. Số lượng luồng hệ điều hành tối đa chạy đồng thời các Goroutines ($N$) được cấu hình tự động bằng số lượng nhân vi xử lý logic (logical CPU cores) của hệ thống thông qua biến môi trường \texttt{GOMAXPROCS} (ví dụ: mặc định $N = 4$ luồng chạy song song tương ứng với cấu hình VPS 4 vCPU được sử dụng để triển khai dự án). Mô hình này giúp tận dụng tối đa năng lực xử lý đa nhân mà không gây ra hao phí hiệu năng do quá trình chuyển đổi ngữ cảnh (context switch) liên tục giữa các luồng.
\end{itemize}
...
\textbf{Mức tiêu thụ tài nguyên} & Trung bình (chạy trên V8 engine). & Cao (tốn RAM và CPU tải thông dịch). & Thấp (tối ưu hóa bộ nhớ tĩnh). \\ \hline
\end{tabular}
...
\begin{itemize}
    ...
    \item \textbf{Lý do lựa chọn Go}: Go kết hợp được cả hai ưu điểm: hiệu năng I/O bất đồng bộ cao như Node.js nhờ cơ chế Network Poller ngầm, và hiệu năng tính toán CPU-bound mạnh mẽ nhờ biên dịch trực tiếp ra mã máy và hỗ trợ đa luồng thực sự \cite{donovan2015go}. Điều này giúp giải quyết triệt để yêu cầu phi chức năng về hiệu năng và độ ổn định của hệ thống máy chủ dưới tải cao.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Tuần tự hóa nhị phân}: Protobuf mã hóa dữ liệu thành định dạng nhị phân nhỏ gọn trước khi truyền trên đường truyền, thay vì dùng dạng văn bản như JSON hay XML.
\end{itemize}
...
Cơ sở dữ liệu của trò chơi trực tuyến lưu trữ các thông tin cốt lõi về tài khoản, tài sản người chơi (Gỗ, Đá, Vàng (Item ID: 00001)), thông tin đệ tử và cấp độ công trình tông môn. Hệ quản trị cơ sở dữ liệu quan hệ PostgreSQL \cite{postgresql2026} được lựa chọn làm kho lưu trữ dữ liệu bền vững.
...
\begin{itemize}
    ...
    \item \textbf{Lý do lựa chọn PostgreSQL}: Trong game chiến thuật thẻ tướng, các giao dịch tài sản yêu cầu tính chính xác và nhất quán tuyệt đối. Ví dụ, khi người chơi thực hiện nâng cấp một công trình tông môn: hệ thống phải kiểm tra số dư tài nguyên, trừ lượng tài nguyên tương ứng (Gỗ, Đá, Vàng (Item ID: 00001)) và tăng cấp độ công trình trong cơ sở dữ liệu. Tất cả các bước này phải nằm trong một Transaction duy nhất. Nếu bất kỳ bước nào lỗi (ví dụ mất kết nối mạng hoặc lỗi server giữa chừng), toàn bộ giao dịch phải được rollback để tránh trường hợp tài nguyên bị trừ nhưng công trình không tăng cấp, hoặc ngược lại. PostgreSQL cung cấp độ tin cậy giao dịch ACID tối đa để giải quyết bài toán này.
\end{itemize}
...
\subsection{Đặc điểm nổi bật}
Redis là hệ thống lưu trữ cấu trúc dữ liệu trong bộ nhớ RAM (in-memory data store) hoạt động dưới dạng key-value với hiệu năng cao (tốc độ đọc ghi hơn 100,000 requests/giây với độ trễ dưới 1 mili-giây). Redis hỗ trợ các cấu trúc dữ liệu nâng cao như String, Hash, List, Set và đặc biệt là Sorted Set.
