...
Các dịch vụ BaaS như Microsoft PlayFab hay Photon Engine thường được các studio game nhỏ lựa chọn \cite{playfab2026} nhờ ưu điểm triển khai nhanh chóng và giảm thiểu gánh nặng tự vận hành phần cứng ở giai đoạn khởi đầu. Tuy nhiên, khi so sánh với giải pháp tự phát triển của đồ án, các dịch vụ này bộc lộ những mặt hạn chế đáng kể:
\begin{itemize}
    ...
    \item \textbf{Độ trễ truyền thông (Network Latency):} Do máy chủ của PlayFab hay Photon thường đặt tại các trung tâm dữ liệu nước ngoài (như Singapore, Mỹ) \cite{playfab2026}, nên các gói tin truyền qua mạng internet quốc tế thường bị ảnh hưởng bởi băng thông tuyến cáp quang biển. Hơn thế nữa, kiến trúc Serverless cốt lõi của các dịch vụ BaaS thường phát sinh thêm độ trễ khởi động (cold-start latency) và overhead đáng kể khi xử lý các trigger API, làm tổng thời gian phản hồi (Response Time) tăng mạnh đối với các hệ thống phân tán \cite{scheuner2020faas}. Theo số liệu đo đạc, điều này khiến tổng độ trễ thực tế từ Việt Nam lên tới 80ms - 200ms+. Trong khi đó, hệ thống tự xây dựng trong đồ án sử dụng giao thức gRPC \cite{indrasiri2020grpc} kết nối trực tiếp đến cụm server nội địa đặt tại Việt Nam, mang lại thời gian phản hồi thực tế thấp (dưới 15ms), tối ưu hóa trải nghiệm người dùng.
    \item \textbf{Khả năng tùy biến sâu và làm chủ mã nguồn:} Sử dụng PlayFab đồng nghĩa với việc lập trình viên phải chấp nhận các giới hạn nghiêm ngặt về mặt logic nghiệp vụ (Business Logic). Các hàm xử lý phía máy chủ (Cloud Script) bị cô lập trong môi trường runtime giới hạn thời gian chạy và dung lượng bộ nhớ. Lập trình viên không thể can thiệp sâu vào nhân cơ sở dữ liệu để thực hiện sharding, điều phối cơ chế cache động hoặc triển khai các thuật toán tùy biến nhằm chống gian lận kết quả chiến đấu (anti-cheat). Với hệ thống tự xây dựng, đồ án làm chủ hoàn toàn mã nguồn, dễ dàng tích hợp các thuật toán xác thực chéo log chiến đấu dựa trên ngưỡng lực chiến, cơ chế ghi nhận tài nguyên lười Lazy Evaluation và cơ chế khóa phân tán Redis Mutex chống tương tranh dữ liệu.
\end{itemize}

\subsubsection{So sánh với kiến trúc máy chủ đơn khối truyền thống (Node.js Monolith + JSON)}

Kiến trúc đơn khối (Monolith) kết hợp Node.js/Express và truyền dữ liệu JSON qua giao thức HTTP/1.1 là giải pháp rất phổ biến do dễ lập trình và cộng đồng hỗ trợ lớn \cite{newman2021building, nodejs2026}. Tuy nhiên, khi hệ thống đòi hỏi khả năng xử lý tương tranh cao và tối ưu hóa tài nguyên phần cứng, kiến trúc này bộc lộ nhiều điểm nghẽn:
\begin{itemize}
    \item \textbf{Hiệu năng xử lý tương tranh (Concurrency):} Node.js vận hành dựa trên cơ chế đơn luồng (Single-Threaded Event Loop). Khi gặp các tác vụ yêu cầu tính toán CPU nặng (chẳng hạn như phân tích cú pháp chuỗi log chiến đấu dài hoặc tính toán ma trận thuộc tính nhân vật), Node.js dễ bị nghẽn xử lý (CPU-bound blocking), làm chậm toàn bộ các yêu cầu khác đang chờ trong hàng đợi. Trong khi đó, ngôn ngữ Go của hệ thống tự xây dựng hỗ trợ tương tranh mức phần cứng thông qua cơ chế Goroutine siêu nhẹ (chỉ tốn khoảng 2KB RAM khởi tạo so với 1-2MB của hệ điều hành Thread) \cite{donovan2015go}. Go có khả năng tận dụng tối đa kiến trúc CPU đa nhân để xử lý đồng thời hàng chục vạn kết nối mà không gây nghẽn hệ thống.
    \item \textbf{Hiệu năng truyền tải và định dạng dữ liệu:} Định dạng JSON truyền qua HTTP/1.1 là định dạng văn bản (text-based), đòi hỏi kích thước gói tin lớn và tiêu tốn nhiều tài nguyên CPU của cả Client lẫn Server cho quá trình mã hóa/giải mã (Serialization/Deserialization). Hệ thống của đồ án sử dụng gRPC truyền tải dữ liệu nhị phân (binary-based) được tuần tự hóa nhanh bằng Protocol Buffers trên nền giao thức HTTP/2 (hỗ trợ ghép kênh Multiplexing). Kích thước các gói tin truyền thông trong game (như Battle Log, thông tin nhân vật) giảm từ 60\% đến 80\% \cite{indrasiri2020grpc}, giúp tiết kiệm đáng kể băng thông đường truyền và giảm tải xử lý cho CPU.
...
\textbf{Độ trễ kết nối (Latency)} & Thấp ($<$15ms) do đặt server nội địa. & Cao (80ms - 200ms) do datacenter ở nước ngoài. & Trung bình đến cao (30ms - 80ms) do dùng HTTP/1.1. \\ \hline
\textbf{Xử lý đồng thời (Concurrency)} & Rất cao (hơn $10^6$ Goroutines đồng thời trên máy chủ từ 8 GB RAM) \cite{donovan2015go}. & Cao, phụ thuộc vào hạ tầng đám mây thuê ngoài. & Trung bình, bị giới hạn bởi cơ chế đơn luồng. \\ \hline
...
\textbf{Băng thông truyền tải} & Thấp (tuần tự hóa nhị phân Protobuf). & Trung bình, phụ thuộc giao thức API của BaaS. & Cao, sử dụng định dạng văn bản JSON qua HTTP. \\ \hline
\end{tabular}
...
\begin{enumerate}
    \item \textbf{Phát triển hoàn chỉnh Game Client (Unity):} Xây dựng ứng dụng máy khách trên nền tảng Unity 6 với hệ thống giao diện thích ứng linh hoạt (Responsive \& Adaptive UI), hiển thị tối ưu và tự động tương thích với tỷ lệ khung hình màn hình của các thiết bị di động và máy tính bảng hiện đại. Tích hợp công nghệ phông chữ vector SDF (Signed Distance Field) giúp duy trì độ sắc nét ở mọi độ phân giải thực tế. Xây dựng bộ phân tích cú pháp nhật ký trận đấu (Battle Log parser) để phát lại (Replay) diễn biến trận đấu sinh động dưới dạng hoạt ảnh 2D, hiệu ứng kỹ năng và âm thanh sống động.
