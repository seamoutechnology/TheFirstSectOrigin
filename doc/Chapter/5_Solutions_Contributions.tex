...
Thực tế này đặt ra một thách thức lớn khi trò chơi "The First Sect Origin" được phát triển để vận hành trên thiết bị di động Android. Hệ thống giao diện cần hiển thị chuẩn xác và cân đối trên nhiều thiết bị có tỷ lệ khung hình (Aspect Ratio) đa dạng trong thực tế:
\begin{itemize}
...
Mọi Canvas giao diện trong trò chơi được cấu hình sử dụng component \texttt{Canvas Scaler} ở chế độ \texttt{Scale With Screen Size}. Độ phân giải tham chiếu (Reference Resolution) được thiết lập tối ưu để tương thích với các tỷ lệ khung hình màn hình di động phổ biến hiện nay. Thuộc tính co dãn phông chữ và hình ảnh được thiết lập dựa trên biến số \texttt{Match Width or Height}. 
...
Ví dụ, đối với khung hiển thị số dư tài nguyên gỗ và vàng (Item ID: 00001) ở góc trên bên phải màn hình, Anchor Min và Anchor Max đều được đặt tại điểm neo \((1.0, 1.0)\), kết hợp với thuộc tính điểm xoay \texttt{Pivot} đặt ở \((1.0, 1.0)\). Cấu hình này đảm bảo khi chiều rộng màn hình thay đổi, khoảng cách từ biên phải màn hình tới khung hiển thị luôn là một hằng số cố định, ngăn chặn hiện tượng khung UI bị trôi vào giữa màn hình hoặc biến mất khỏi cạnh phải.
...
\begin{itemize}
    \item Giao diện của trò chơi hiển thị hoàn chỉnh, cân đối và sắc nét trên mọi loại thiết bị di động cũng như máy tính bảng hiện đại mà không cần viết các đoạn mã căn chỉnh riêng biệt cho từng thiết bị. Phông chữ và các phần tử UI tự động thích ứng với sự thay đổi của kích thước màn hình thực tế.
...
\section{Cơ chế xử lý đồng thời và đồng bộ hóa tài nguyên nhàn rỗi (Idle Resource Generation) đảm bảo nhất quán}
\label{sec:sol_idle_resource}

\subsection{Dẫn dắt và giới thiệu bài toán}
Trong các trò chơi trực tuyến thể loại mô phỏng xây dựng tông môn (như "The First Sect Origin"), cơ chế sản sinh tài nguyên nhàn rỗi (Idle Resource Generation) là một tính năng cốt lõi. Người chơi xây dựng các công trình khai thác (như Lâm trường sản xuất Gỗ, Mỏ vàng (Item ID: 00001) sản xuất Vàng (Item ID: 00001)) và các công trình này liên tục sản sinh tài nguyên sau mỗi giây, tích lũy vào kho chứa ngay cả khi người chơi đã thoát game (offline).
...
Bên cạnh đó, khi người chơi gửi yêu cầu thu hoạch tài nguyên, nếu họ bấm liên tiếp phím thu hoạch trong thời gian ngắn từ nhiều thiết bị hoặc gửi nhiều yêu cầu đồng thời (Race Condition), hệ thống backend vi dịch vụ có thể thực hiện cộng tài nguyên hai lần trước khi kịp ghi nhận mốc thời gian đã thu hoạch mới (Double Collect), dẫn đến thất thoát tài nguyên và phá vỡ nền kinh tế trong game.

\subsection{Giải pháp đề xuất}
Để giải quyết triệt để bài toán này, đồ án đã thiết kế và hiện thực hóa một cơ chế đồng bộ hóa tài nguyên nhàn rỗi nhất quán dựa trên ba trụ cột kỹ thuật:
...
success, err := redisClient.SetNX(
    ctx, lockKey, "locked", 5*time.Second,
).Result()
\end{lstlisting}
...
\begin{itemize}
    \item Nếu \texttt{success} trả về \texttt{true}, luồng xử lý hiện tại chiếm được khóa và thực hiện tiếp tiến trình tính toán và ghi dữ liệu vào PostgreSQL. Sau khi cập nhật DB thành công, luồng xử lý sẽ gọi một đoạn script Lua để xóa khóa Redis.
...
\begin{itemize}
    \item **Đảm bảo bảo mật**: Loại bỏ hoàn toàn lỗ hổng Double Collect. Thử nghiệm gửi đồng thời 100 yêu cầu thu hoạch giả lập trong vòng 50ms cho thấy duy nhất 1 yêu cầu đầu tiên được xử lý thành công, 99 yêu cầu còn lại bị chặn đứng tại bộ nhớ đệm Redis với thời gian phản hồi nhanh dưới 2ms.
    \item **Tối ưu hóa tài nguyên cơ sở dữ liệu**: Loại bỏ hoàn toàn gánh nặng ghi liên tục của tiến trình ngầm. Lượng câu lệnh UPDATE ghi vào database giảm xuống tối đa chỉ bằng số lần người chơi thực sự tương tác click thu hoạch (trung bình giảm từ 3600 câu lệnh ghi/giờ xuống còn 2-3 câu lệnh ghi/giờ cho mỗi người chơi hoạt động), giúp PostgreSQL duy trì mức sử dụng CPU thấp dưới 15\% ngay cả khi hệ thống chạy giả lập tải đỉnh 5.000 CCU.
\end{itemize}
...
Khi nhận được yêu cầu xác thực, vi dịch vụ \texttt{Combat Server} viết bằng Go (tại file \texttt{combat\_service.go}) sẽ thực hiện thuật toán kiểm tra logic:
\begin{enumerate}
    ...
    \item \textbf{Phân loại theo ngưỡng lực chiến}: 
      ...
      $\bullet$ Nếu lực chiến của người chơi thấp bất thường (\(\text{PlayerPower} < 0.5 \times \text{EnemyPower}\) - trường hợp nghi vấn hack chỉ số để vượt ải khó), server kích hoạt hàm xác thực nhật ký \texttt{verify\allowbreak CombatLog}.
      
    \item \textbf{Xác thực cấu trúc nhật ký trận đấu}: Hàm \texttt{verify\allowbreak CombatLog} duyệt qua mảng \texttt{CombatLogs} gửi lên từ Client để kiểm tra hai ràng buộc logic:
...
\subsubsection{Trích xuất nhật ký hệ thống (Real-time Server Logs)}
Dưới đây là trích xuất từ màn hình giám sát (Terminal) của vi dịch vụ Combat Server. Phần đầu tiên là bản ghi thực tế của một **trận đấu hợp lệ**, cho thấy hệ thống ghi nhận chính xác từng hành động của các nhân vật. Hai phần tiếp theo minh họa kịch bản hệ thống phát hiện và ngăn chặn tự động hai trường hợp người chơi cố tình gửi dữ liệu gian lận:
...
{=======================================================}\\
{[SERVER LOG] PvE VALIDATION REQUEST RECEIVED}\\
{- Player Power: 4307 | Enemy Power: 2070}\\
{- Reported Victory: true | Logs Count: 24}\\
{--- COMBAT LOGS START ---}\\
{  [01] Caster:Hắc Sát Thần -> Target:Quái thú hoang dã | Skill:skill\_shadow\_strike | Dmg:314   | Crit:false}\\
{  [02] Caster:Quái thú hoang dã -> Target:Thủy Hộ Thuẫn | Skill:enemy\_attack        | Dmg:72    | Crit:true}\\
{  [04] Caster:Hắc Sát Thần -> Target:Quái thú hoang dã | Skill:master\_skill\_lightning| Dmg:437   | Crit:false}\\
{  ...}\\
{  [24] Caster:Lửa Chiến Binh -> Target:Quái thú hoang dã| Skill:skill\_slash         | Dmg:144   | Crit:false}\\
{--- COMBAT LOGS END ---}\\
{=======================================================}\\
\\
{[INFO] 2026-06-24 15:30:10 [PVE] Player 10442 validation request received.}\\
{[WARN] 2026-06-24 15:30:10 [PVE] Suspect combat! PlayerPower(1200) << EnemyPower(5500). Triggering Deep Scan...}\\
{[ERROR] 2026-06-24 15:30:10 [SECURITY] HACK DETECTED: Empty combat logs but Victory reported.}\\
{[ERROR] 2026-06-24 15:30:10 [SECURITY] -> Action: REJECT\_BATTLE\_RESULT. PlayerID: 10442.}\\
\\
{[INFO] 2026-06-24 15:35:22 [PVE] Player 89912 validation request received.}\\
{[WARN] 2026-06-24 15:35:22 [PVE] Suspect combat! PlayerPower(3400) vs EnemyPower(9800). Triggering Deep Scan...}\\
{[ERROR] 2026-06-24 15:35:22 [SECURITY] HACK DETECTED: Max Single Hit Limit Exceeded!}\\
{[ERROR] 2026-06-24 15:35:22 [SECURITY] -> Damage hit: 999999. Max Possible Limit: 6800.}\\
{[ERROR] 2026-06-24 15:35:22 [SECURITY] -> Action: REJECT\_BATTLE\_RESULT \& BAN\_WARNING. PlayerID: 89912.}
\end{minipage}
}
\vspace{0.3cm}

\subsubsection{Cập nhật tiến trình và trao thưởng tại World Server}
Nếu kết quả xác thực hợp lệ (\texttt{IsValid = true}), \texttt{World Server} thực hiện lưu tiến trình vượt ải mới của người chơi vào PostgreSQL Game Shard DB, cộng điểm kinh nghiệm, tài nguyên gỗ/vàng (Item ID: 00001) thưởng vào tài sản người chơi và phản hồi thành công về Client. Nếu không hợp lệ, server trả về mã lỗi từ chối và ngăn chặn việc cộng thưởng.
...
\begin{itemize}
    ...
    \item **Tối ưu hóa hiệu năng máy chủ**: Nhờ cơ chế chỉ xác thực các trường hợp chiến thắng nghi vấn (lực chiến thấp vượt ải khó) và kiểm tra duyệt mảng log trên bộ nhớ đệm RAM nhanh (không cần chạy lại tiến trình mô phỏng trận đấu), thời gian xử lý xác thực trên máy chủ chỉ mất dưới \(1\text{ms}\). Điều này giúp hệ thống backend hoạt động ổn định, duy trì với tỷ lệ lỗi request đạt mức 0.0\% dưới tải đỉnh 5.000 CCU, và mức tiêu thụ RAM của Combat Server chỉ duy trì dưới 150MB, giúp tối ưu hóa chi phí vận hành phần cứng thực tế.
\end{itemize}
